...
def __main__():
	...
	output.writelines( 'Query\tBest Hit\tQ.start\tQ.end\tS.start\tS.end\tAlignment Length\tIdentity\tBit Score\tE-value\tHit Description\n' )
	try:
		for blast_record in blast_records:
			for alignment in blast_record.alignments:
				...
				for hsp in alignment.hsps:
					...
					#+ str(hsp.identities) + '\t' \
					+ str( round(float(hsp.identities)/hsp.align_length, 4)*100 ) + '%\t' \
					+ str(hsp.bits) + '\t' \
					+ str(hsp.expect) + '\t' + alignment.hit_def + '\n' )
	except:
		print 'error'
	
	print "Delete the duplication rows by the first column, and write the output into %s!" % final_output
	subprocess.call( "awk -F '\t' '$1 == cust_id{} $1 != cust_id{cust_id = $1;print $0}' " + output_file + ' > ' + final_output, stdout=subprocess.PIPE, shell=True )
	...
	for line in open(final_output):
		if line.startswith('Query'):
			...
			taxa_output.write(re.split('\s+',line_list[0])[0]+'\t'+line_list[-1]+'\t'+line_list[-4]+'\n')
	taxa_output.close()
