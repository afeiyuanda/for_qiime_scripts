...
def change_line_taxa_flag(line):
	...
	if line_list[0].split(';')[-1] in unknown_list:
		...
		taxa_name = line_list[0].split(';')[-1]
		if p.match(taxa_name):
			taxa_name = p.match(taxa_name).groups()[0] + p.match(taxa_name).groups()[1]
		new_line = taxa_name + '\t' + '\t'.join(line_list[1:]) + '\n'
	return new_line
...
def generate2lines_blocks( lines ):
	...
	known_taxa_blocks = []
	unknown_taxa_blocks = []
	unknown_list = ['p__','c__','o__','f__','g__','Other','p__unidentified','c__unidentified','o__unidentified','f__unidentified','g__unidentified','p__Incertae_sedis','c__Incertae_sedis','o__Incertae_sedis','f__Incertae_sedis','g__Incertae_sedis']
	known_taxa_dic = {}
	for line in lines:
...
def __main__():
	...
	for taxa_file in taxa_file_list:
		...
		if sample_number <= 10:
			...
			subprocess.call( 'perl '+sys.path[0]+'/Rscripts/bar_pie_less_10sample.pl -i ' + level + '.txt -pie F -bw 4 -bh 4', stdout=subprocess.PIPE, shell=True )
			subprocess.call( 'rm -rf *.r *.Rout', stdout=subprocess.PIPE, shell=True )
			#os.chdir('../../')
		else:
			os.chdir(new_taxa_file_dir)
			subprocess.call( 'perl '+sys.path[0]+'/Rscripts/bar_pie.pl -i ' + level + '.txt -pie F -bw 4 -bh 4', stdout=subprocess.PIPE, shell=True )
			subprocess.call( 'rm -rf *.r *.Rout', stdout=subprocess.PIPE, shell=True )
			#os.chdir('../../')
