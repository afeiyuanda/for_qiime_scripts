...
Author:	Liu wei
Data:	2015.03.18
Usage:	python grep_from_miseq_fastq_with_part_barcode_20150318.py  input_seqs_ul.fastq  mapping.txt edit_output.fastq
...
def primer_list( ul ):
	...
	for line in lines[1:]:
		...
		primer.append( temp[2].strip() ) #forward primer,primer[3]
...
def usage():
    print """
		...
		20150318 modify: use part of barcode to grep seqs!
...
def get_substr_pos( full_str, sub_str ):
	try:
...
def __main__():
	...
	grep_seqs_log = 'grep_seqs_num_log.txt'
	...
	for seq_id, seq, qual  in MinimalFastqParser(input_seqs_ul, strict=False):
		...
		for primer in primers:
			...
			flag_seq =  primer[1]
			if flag_seq in seq[0:11]:
				outf.write('@%s\n%s\n+\n%s\n' % (seq_id, seq[seq.index(flag_seq):], qual[seq.index(flag_seq):]))
				grep_seqs_dic[ primer[0] ] = grep_seqs_dic[ primer[0] ] + 1
				matched_seqs = matched_seqs + 1
				break
			elif reverse_com_seq(flag_seq) in seq[-11:]:
				fpart_seq = seq[rprimer_len:-11]
				rpart_seq = seq[-11:]
				new_rpart = rpart_seq[ :rpart_seq.index(reverse_com_seq(flag_seq)) ]
				new_seq = flag_seq + reverse_com_seq( fpart_seq + new_rpart )
	...
	lib_name = input_seqs_ul.split('.')[0]
