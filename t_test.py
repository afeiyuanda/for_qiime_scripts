	...
	for level_file in level_files:
		...
		for data in data_list:
			...
			while i < len( data_list ):
				p_value_list.append( 'mydata$Group_' + data + '_' + data_list[i] + '_p_value[i]<-t.test(data.matrix(data' + data + '),data.matrix(data' + data_list[i] + '),alternative="two.sided",exact=FALSE,correct=FALSE)[[3]]\n' )
		...
		final_test_output = 'average_t_test_' + '_'.join( groups_dic.keys() ) + '.xls'
