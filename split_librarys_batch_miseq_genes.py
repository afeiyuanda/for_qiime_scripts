...
def generate_lines_blocks(lines, clo_num):
	...
	flag_list = []
	for line in lines:
		flag = re.split('\s+', line.rstrip())[int(clo_num)-1]
...
def __main__():
	...
	for lib_name_list in big_lib_name_list:
		...
		if int(primer_flag) == 0:
			cmd = 'python ' + sys.path[0] + '/grep_seqs_by_barcode/grep_from_miseq_fastq_for_genes.py ' + \
		...
		#subprocess.call( 'identify_chimeric_seqs.py -i '+output_dir+'/seqs.fna -m usearch61 --suppress_usearch61_ref -o '+seqs_dir+'/'+f1+'/usearch_checked_chimeras', stdout=subprocess.PIPE, shell=True )
	...
	subprocess.call( 'cat '+seqs_dir+'/*/split_library_output/seqs.fna > '+outdir+'/final_seqs.fna', stdout=subprocess.PIPE, shell=True )
