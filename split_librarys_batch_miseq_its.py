...
def __main__():
	...
	lines = f.readlines()
	f.close()
	barcode_len = re.split('\s+', len(lines[1])[1])
	#primer_flag==0:Contains Paired Primer
	#primer_flag==2:No Primer
	#primer_flag==1:Single Primer
	...
	for lib_name_list in big_lib_name_list:
		...
		subprocess.call( 'identify_chimeric_seqs.py -i '+output_dir+'/seqs.fna -m usearch61 -o '+seqs_dir+'/'+f1+'/usearch_checked_chimeras -r $its_reference_seqs', stdout=subprocess.PIPE, shell=True )
