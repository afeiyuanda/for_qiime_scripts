...
$opts{ncol}=defined $opts{ncol}?$opts{ncol}:3;
...
if(bar ==\"T\"){
	...
	text(cex=0.4,x=bar_plot, y=-2, xpd=TRUE, lab=colnames(otu), adj=0.5) # srt control the angle, y control the distance between column name and the x axis
	...
	mtext(1, text = \"Sample ID\", line = -0.1, cex=0.3) #cex control the font size, line control the distance between text and the x axis
