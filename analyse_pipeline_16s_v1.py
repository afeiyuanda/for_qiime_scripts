...
def __main__():
	...
	pick_otu_handle.write('biom  summarize-table -i '+biom+' -o '+outdir+'/seqs_per_sample.txt\n' )
	pick_otu_handle.write('biom  summarize-table -i '+biom+' --qualitative -o '+outdir+'/otus_per_sample.txt\n' )
	...
	for group in groups_list:
		...
		alpha_handle.write('sort_otu_table.py -i '+biom+' -m '+mapping+' -s '+group+' -o '+outdir+'/sorted_otu_table_'+group+'.biom\n')
		...
		alpha_handle.write('parallel_alpha_diversity.py -i '+outdir+'/alpha_diversity_'+group+'/rarefied_otu_tables -m ACE,simpson,shannon,PD_whole_tree,chao1,observed_species,goods_coverage -o '+outdir+'/alpha_diversity_'+group+'/adiv/ -t '+tre+' -O '+str(cpu)+'\n')
		...
		beta_handle.write('parallel_beta_diversity.py -i '+outdir+'/beta_diversity/otu_table_even.biom -t '+tre+' -o '+outdir+'/beta_diversity/beta_div -O '+str(4)+'\n')
		...
		beta_handle.write('alpha_diversity.py -i '+outdir+'/beta_diversity/otu_table_even.biom -m ACE,simpson,shannon,PD_whole_tree,chao1,observed_species,goods_coverage -o '+outdir+'/even_adiv.txt -t '+tre+'\n')
		...
		beta_handle.write('jackknifed_beta_diversity.py -i '+outdir+'/sorted_otu_table_'+group+'.biom -t '+tre+'  -m '+mapping+' -o '+outdir+'/wf_jack -e '+str(int(min_seqs_num*0.95))+'\n')
